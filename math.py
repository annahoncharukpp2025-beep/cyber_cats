#!/usr/bin/env python3
"""
ArduPilot DataFlash (.BIN) parser focused on GPS + IMU.

What this script fixes compared with many "looks stationary" implementations:
1) GPS is filtered by validity (Status/NSats), so broken first samples do not explode trajectory.
2) Lat/Lng are converted to local ENU meters, so movement is shown in meters instead of tiny degree changes.
3) Haversine is used for traveled distance, as required.
4) IMU instances are not mixed together.
5) IMU acceleration is rotated from body frame to navigation frame using ATT roll/pitch/yaw
   and gravity is subtracted before trapezoidal integration.
6) Metrics are exported in a structured, analysis-ready form.

Requires:
    pip install pymavlink pandas numpy plotly pyarrow

Usage:
    python ardupilot_telemetry_parser.py /path/to/00000001.BIN --out ./out
...
Outputs:
    out/
      ...
      trajectory_3d.html
      speed_altitude.html
      imu_accel_velocity.html
      gps.parquet / imu.parquet / att.parquet   (if pyarrow available)
"""
...
try:
    ...
    raise SystemExit(
        "pymavlink is required. Install inside your venv:\n"
        "  python -m pip install pymavlink pandas numpy plotly pyarrow\n"
...
def msg_to_dict(msg: Any) -> Dict[str, Any]:
    """
    Convert a pymavlink DataFlash message into a plain dict.
    Tries several methods because pymavlink objects differ slightly by version.
    """
    ...
    out: Dict[str, Any] = {}
    # _fieldnames is the most common attribute on MAVLink messages
...
def iter_bin_messages(bin_path: str | os.PathLike[str]) -> Iterable[Dict[str, Any]]:
    """
    Iterate through DataFlash BIN messages as dicts.
    """
    ...
    while True:
        ...
        if msg is None:
            break
        try:
            record = msg_to_dict(msg)
            yield record
        except Exception:
            continue


def collect_message_tables(bin_path: str | os.PathLike[str],
    ...
    for mtype, rows in tables.items():
        dfs[mtype] = pd.DataFrame(rows) if rows else pd.DataFrame()
    return dfs
...
def choose_primary_instance(df: pd.DataFrame, instance_col: str = "I") -> Tuple[pd.DataFrame, int | None]:
    """
    Keep only the instance with the most rows.
    This avoids mixing IMU0 + IMU1 or GPS0 + GPS1 in one timeline.
    """
...
def ecef_to_enu(x: np.ndarray, y: np.ndarray, z: np.ndarray,
                lat0_deg: float, lon0_deg: float, alt0_m: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def preprocess_gps(gps_raw: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    # Remove obviously broken coordinates
    gps = gps[
    ...
    # GPS validity filter: avoid the classic bad first sample problem
...
def preprocess_att(att_raw: pd.DataFrame, ahr2_raw: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    # Keep AHR2 quaternion fields if present, but this script currently uses ATT Euler angles.
    meta = {"att_source": source}
    if att.empty:
        meta["warning"] = "No ATT messages found; IMU integration will stay in body frame if ATT is absent."
...
def body_to_nav_rotation_matrix(roll_rad: np.ndarray, pitch_rad: np.ndarray, yaw_rad: np.ndarray) -> np.ndarray:
    """
    Rotation matrix body -> navigation (NED-like orientation assumptions are implementation-dependent in FC logs).
    Here we use a standard ZYX yaw-pitch-roll composition to rotate body vectors into a navigation frame.
    """
...
def trapz_integrate_vector(time_s: np.ndarray, values: np.ndarray) -> np.ndarray:
    """
    Trapezoidal integration for vector-valued series.
    values shape: (N, 3)
    """
...
def preprocess_imu(imu_raw: pd.DataFrame, att: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    # Rotate body -> navigation
    R = body_to_nav_rotation_matrix(roll_rad, pitch_rad, yaw_rad)
    acc_nav = np.einsum("nij,nj->ni", R, acc_body)

    # Gravity compensation:
    # We assume navigation frame "up" is +Z, so subtract gravity from Z after rotation.
    ...
    imu["acc_nav_x_mps2"] = acc_nav[:, 0]
    imu["acc_nav_y_mps2"] = acc_nav[:, 1]
    imu["acc_nav_z_mps2"] = acc_nav[:, 2]
...
def infer_flight_window_from_gps(gps: pd.DataFrame) -> Tuple[float, float]:
    """
    Conservative window: first and last valid GPS samples.
    This is much safer than using the full BIN log duration because ArduPilot can log while disarmed.
    """
    if gps.empty:
        return np.nan, np.nan
    return float(gps["time_s"].iloc[0]), float(gps["time_s"].iloc[-1])
...
def compute_metrics(gps: pd.DataFrame, imu: pd.DataFrame) -> Dict[str, Any]:
    ...
    if not gps.empty:
        t0, t1 = infer_flight_window_from_gps(gps)
        metrics.update({
            "flight_start_s": t0,
            "flight_end_s": t1,
            "flight_duration_s": t1 - t0,
            "gps_total_distance_m": float(gps["seg_m"].sum()),
            "gps_horizontal_displacement_m": float(np.hypot(gps["east_m"].iloc[-1], gps["north_m"].iloc[-1])),
            ...
            "max_altitude_gain_m": float(gps["alt_m"].max() - gps["alt_m"].iloc[0]),
...
def write_plots(gps: pd.DataFrame, imu: pd.DataFrame, out_dir: Path) -> None:
    try:
        import plotly.express as px
        import plotly.graph_objects as go
    except Exception:
        print("Plotly not installed, skipping HTML plots.")
        return
    ...
    if not gps.empty:
        traj = px.scatter_3d(
            ...
            title="3D trajectory in local ENU coordinates",
        )
        traj.update_traces(marker=dict(size=3))
        traj.write_html(out_dir / "trajectory_3d.html")
        ...
        fig.update_layout(
            title="Altitude and horizontal speed vs time",
            xaxis_title="Time [s]",
            yaxis_title="Altitude [m]",
            yaxis2=dict(title="Speed [m/s]", overlaying="y", side="right"),
        )
        fig.write_html(out_dir / "speed_altitude.html")

    if not imu.empty:
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=imu["time_s"], y=imu["acc_lin_norm_mps2"], mode="lines", name="Linear acceleration norm [m/s²]"))
        fig.add_trace(go.Scatter(x=imu["time_s"], y=imu["vel_est_norm_mps"], mode="lines", name="Integrated speed norm [m/s]", yaxis="y2"))
        fig.update_layout(
            title="IMU linear acceleration and trapezoid-integrated speed",
            xaxis_title="Time [s]",
            yaxis_title="Linear acceleration [m/s²]",
            yaxis2=dict(title="Integrated speed [m/s]", overlaying="y", side="right"),
        )
        fig.write_html(out_dir / "imu_accel_velocity.html")


# ---------------------------------- export -----------------------------------


def maybe_write_parquet(df: pd.DataFrame, path: Path) -> Optional[str]:
    if df.empty:
        return None
    try:
        ...
        return f"Parquet skipped: {exc}"


def save_outputs(gps: pd.DataFrame, imu: pd.DataFrame, att: pd.DataFrame,
                 metadata: Dict[str, Any], metrics: Dict[str, Any], out_dir: Path) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)

    if not gps.empty:
        gps.to_csv(out_dir / "gps.csv", index=False)
        maybe_write_parquet(gps, out_dir / "gps.parquet")

    if not imu.empty:
        imu.to_csv(out_dir / "imu.csv", index=False)
        maybe_write_parquet(imu, out_dir / "imu.parquet")

    if not att.empty:
        att.to_csv(out_dir / "att.csv", index=False)
        maybe_write_parquet(att, out_dir / "att.parquet")
    ...
    write_plots(gps, imu, out_dir)


# ----------------------------------- main ------------------------------------


def process_log(bin_path: str | os.PathLike[str], out_dir: str | os.PathLike[str]) -> Dict[str, Any]:
    ...
    metadata = {
        ...
        "imu": imu_meta,
        "notes": [
            "GPS trajectory is shown in local ENU meters from the first valid GPS point.",
            "GPS traveled distance is computed with haversine.",
            "IMU speed estimate is computed from gravity-compensated acceleration using trapezoidal integration.",
            "IMU integrated speed will drift over time; this is expected for pure inertial integration.",
        ],
    }

    metrics = compute_metrics(gps, imu)
    save_outputs(gps, imu, att, metadata, metrics, Path(out_dir))
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Parse ArduPilot DataFlash BIN into GPS/IMU tables and mission metrics.")
    p.add_argument("bin_file", help="Path to .BIN file")
...
def main() -> None:
    ...
    result = process_log(args.bin_file, args.out)
